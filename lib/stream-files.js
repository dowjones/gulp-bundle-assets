...
module.exports.handleTransformError = function (thisStream, isWatch, bundleName, bundleKey, err) {
  logger.log(colors.red("ERROR in custom transforms for '" + bundleName + "." + bundleKey + "':"));
...
module.exports.scripts = function (opts) {
  ...
  return gulp.src(opts.src, {base: opts.base})
    ...
    .pipe(gif(isOptionEnabled(opts.bundleOptions.rev),
      gif(function (file) {
        return file.isStream();
      }, streamify(rev()), rev())
    ))
    ...
    .pipe(addBundleResultsToFile(opts.bundleName, BundleKeys.SCRIPTS, opts.bundleOptions.result, opts.bundleOrder, relativeSources));
};

module.exports.styles = function (opts) {//error somewhere here
  var self = this;
  var relativeSources = [];
  return gulp.src(opts.src, {base: opts.base})
    .pipe(using.bundle(opts.bundleName, BundleKeys.STYLES))
    .pipe(through.obj(function (file, enc, cb) {
      relativeSources.push(file.relative);
      self.attachStreamOptions(file, opts);
      this.push(file);
    ...
    .pipe(cleanCSS(opts.bundleOptions.pluginOptions['gulp-clean-css']))
    .pipe(gif(function (file) {
        return opts.bundleOptions.order && opts.bundleOptions.order.styles;
      }, order(opts.bundleOptions.order.styles)
    ))
    .pipe(concatCss(opts.bundleName + '.css'))
    .pipe(gif(isOptionEnabled(opts.bundleOptions.rev), rev()))
    .pipe(gif(function (file) {
        return sourcemaps.isEnabled(opts);
      }, gsourcemaps.write(
    ...
    .pipe(addBundleResultsToFile(opts.bundleName, BundleKeys.STYLES, opts.bundleOptions.result, opts.bundleOrder, relativeSources));
};
