...
function objectSrc(src, base, options) {
  if (options && !src.src && options.useMin === false) {
    ...
    if (!src.src) {
      ...
      return arrayOrStringSrc(src.minSrc, base, options);
    }
  } else if (options.useMin) {
    if (typeof options.useMin === 'string') {
      if (options.useMin === process.env.NODE_ENV && src.minSrc) {
        return arrayOrStringSrc(src.minSrc, base, options);
      }
      return arrayOrStringSrc(src.src, base, options);
    } else if (util.isArray(options.useMin)) {
      if (contains(options.useMin, process.env.NODE_ENV) && src.minSrc) {
        return arrayOrStringSrc(src.minSrc, base, options);
      }
      return arrayOrStringSrc(src.src, base, options);
    } else if (!src.minSrc) {
      return arrayOrStringSrc(src.src, base, options); // for array notation allow some to be defined
    }
    return arrayOrStringSrc(src.minSrc, base, options);
  }
  return arrayOrStringSrc(src.src, base, options);
...
module.exports = function (src, base, options) {
  if (typeof src === 'string' || util.isArray(src)) {
    return arrayOrStringSrc(src, base, options);
  } else if (src && typeof src === 'object') { // typeof null === 'object'
    return objectSrc(src, base, options);
  }
  throw new Error(ERROR_MSG_PREFIX + 'Invalid bundle glob detected. Expected string, array or object but got ' + src);
};