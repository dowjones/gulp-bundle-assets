    ...
    _.forEach(Object.keys(namedBundleObj), function (type) {
      ...
      if (type === BundleKeys.SCRIPTS) {
        ...
        if (scriptWatch !== false) {
          ...
          logger.log("Starting '" + colors.cyan("watch") + "' for bundle '" + colors.green(prettyScriptsBundleName) + "'...");
        ...
        if (styleWatch !== false) {
          ...
          logger.log("Starting '" + colors.cyan("watch") + "' for bundle '" + colors.green(prettyStylesBundleName) + "'...");
...
function _copy(config) {
  ...
  logger.log("Starting '" + colors.cyan("watch") + "' for files to copy...");
...
function bundle(config) {
  ...
  if (config.bundle) _bundle(config);

  streamBundlesUtil.warnIfNoBundleProperty(config);

  if (config.copy) _copy(config);
}
