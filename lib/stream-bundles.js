var fs = require('fs'),
  ...
  errPrefix = gutil.colors.bgRed.white('ERR!');
...
function checkForEmptySourcemaps(file, enc, cb) {
  if (!file.sourceMap.sources.length) {
    ...
    gutil.log("\n" +
      errPrefix + " Source map is empty for file '" + gutil.colors.magenta(file.relative) + "'.\n" +
      errPrefix + " This is most likely not a problem with '" + gutil.colors.cyan('gulp-bundle-assets') + "' itself.\n" +
      errPrefix + " This usually happens when a file passing through the stream is invalid or malformed\n" +
      errPrefix + " or if you have a misbehaving custom transform.\n");
  }
  this.push(file);
  cb();
}
    ...
    _.forEach(Object.keys(namedBundleObj), function (type) {
      ...
      if (type === BundleKeys.SCRIPTS) {
        ...
        streams.push(gulp.src(pathifySrc(namedBundleObj[BundleKeys.SCRIPTS], base, namedBundleObj[BundleKeys.OPTIONS], env), {base: base})
          .pipe(using.bundle(bundleName, BundleKeys.SCRIPTS, env, isBundleAll))
          .pipe(sourcemaps.init({loadMaps: true}))
          .pipe(namedBundleObj[BundleKeys.OPTIONS].transforms[BundleKeys.SCRIPTS]())
          .pipe(through.obj(checkForEmptySourcemaps))
          ...
          .pipe(addBundleResultsToFile(bundleName, BundleKeys.SCRIPTS, namedBundleObj[BundleKeys.OPTIONS].result, env, isBundleAll)));
        ...
        streams.push(gulp.src(pathifySrc(namedBundleObj[BundleKeys.STYLES], base, namedBundleObj[BundleKeys.OPTIONS], env), {base: base})
          .pipe(using.bundle(bundleName, BundleKeys.STYLES, env, isBundleAll))
          .pipe(sourcemaps.init({loadMaps: true}))
          ...
          .pipe(gif(isOptionEnabled(namedBundleObj[BundleKeys.OPTIONS].rev, env), rev()))
          .pipe(sourcemaps.write('maps'))
          .pipe(addBundleResultsToFile(bundleName, BundleKeys.STYLES, namedBundleObj[BundleKeys.OPTIONS].result, env, isBundleAll)));
...
function bundle(config) {
  ...
  if (config.bundle) {
    ...
    gutil.log(warnPrefix, "No '" + gutil.colors.cyan('bundle') +
