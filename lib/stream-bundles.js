      ...
      if (type === BundleKeys.SCRIPTS) {
        streams.push(gulp.src(pathifySrc(namedBundleObj[BundleKeys.SCRIPTS], base, namedBundleObj[BundleKeys.OPTIONS], env), {base: base})
          ...
          .pipe(gif(function (file) {
            ...
            streamify(concat(bundleName + ((isBundleAll && env) ? '.' + env : '') + '.js')),
            concat(bundleName + ((isBundleAll && env) ? '.' + env : '') + '.js')
          ))
          .pipe(gif(isOptionEnabled(namedBundleObj[BundleKeys.OPTIONS].rev, env),
            gif(function (file) {
              return file.isStream();
            }, streamify(rev()), rev())
          ))
