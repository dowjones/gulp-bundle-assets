var fs = require('fs'),
  ...
  warnPrefix = chalk.bgYellow.black('WARN'),
  errPrefix = chalk.bgRed.white('ERR!');
...
function checkForEmptySourcemaps(file, enc, cb) {
  if (!file.sourceMap.sources.length) {
    ...
    gutil.log("\n" +
      errPrefix + " Source map is empty for file '" + chalk.magenta(file.relative) + "'.\n" +
      errPrefix + " This is most likely not a problem with '" + chalk.cyan('gulp-bundle-assets') + "' itself.\n" +
...
function bundle(config) {
  ...
  if (config.bundle) {
    ...
    gutil.log(warnPrefix, "No '" + chalk.cyan('bundle') +
      "' property found in " + chalk.magenta(config.file.relative) + ". Did you mean to define one?");
  }
